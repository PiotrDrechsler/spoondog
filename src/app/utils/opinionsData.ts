export const opinionsData = [
  {
    name: "Krzysztof D.",
    opinion:
      "Piesek ostrzyzony, wykapany, pachnacy i to wszystko nie za miliony monet! 🤩👍 Poprawki zrobione na moje zyczenie bezproblemowo. Kazdy przeciez ma swoja wizje. Na pewno wrocimy! ❤️❤️",
  },
  {
    name: "Pawel S.",
    opinion:
      "Świetne miejsce / bardzo dobre ceny / fachowa i miła obsluga. Piesek szczęśliwy lżejszy o zimowe futro",
  },
  {
    name: "Andrzej H.",
    opinion: "Super profesjonalna, szybka i miła obsługa klienta. Polecam",
  },
  {
    name: "Lukas L.",
    opinion: "Daisy dziękuję za piękne obcięcie!",
  },
  {
    name: "Szymon W.",
    opinion: "Korzystam od kilkunastu lat. Jestem bardzo zadowolony.",
  },
  {
    name: "Mati G.",
    opinion:
      "Najlepsze miejsce dla mojego FAFIKA ;) Zawsze zadowolony wychodzi z tamtad... Pozdrawiamy i polecamy gorąco!!",
  },
  {
    name: "Damian J.",
    opinion:
      "Pełna profeska, pies zadowolony, to my też. Napewno będziemy wracać, bo była to nasza 1 wizyta. A była z polecenia",
  },
  {
    name: "Izabela W.",
    opinion:
      "Pani bardzo miła z dobrym podejściem do pupili. Zawsze wychodzimy zadowoleni. Polecam",
  },
  {
    name: "Anna B.",
    opinion: "Szybka, miła i profesjonalna obsługa. Polecam ! 🥰",
  },
  {
    name: "Kamil K.",
    opinion: "Polecam! Świetne podejście. Pełen profesjonalizm.",
  },
];
